...
class FlashUI(wx.Frame):
    def __init__(self,title,pos,size):
        ...
        self.dl_list = [[u"D:\\flash_one_firmware_hadata_tool\\bin", 0],['data csv',0x78000],['Sign a sector', 0x7A000]]
...
3723*'\xFF']
    ...
    def find_bin_from_directory(self,path_name, file_name):  # 从目录中寻找指定文件，返回文件的路径
        ret_path = ''
        ...
        try:
            for filename in os.listdir(path_name):
                ...
                if os.path.isfile(fp) and file_name in filename:
                    ...
                    ret_path = self.find_bin_from_directory(fp, file_name)
                    if (ret_path != '') and (ret_path != None):
                        return ret_path
        except Exception:
            print "find bin error"
            ...
            if ret:
                for filename, offset in self.dl_list:
                    if filename.find("csv") != -1:
                        ...
                        file_name = self.find_bin_from_directory(filename,".bin")
                        print "bin:%s"%file_name
                        print >>self.f_note,"bin:%s"%file_name
...
if __name__ == '__main__':
    ...
    app = wx.App()
    frame = FlashUI(u'Flash_stub_one_firmware1.1',(100,150),(550,550))
    frame.Show(True)
    app.MainLoop()